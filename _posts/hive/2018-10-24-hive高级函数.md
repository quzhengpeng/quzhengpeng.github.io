...
将多行记录合并成一行。CONCAT_WS() 代表 Concat With Separator，是CONCAT() 的特殊形式。第一个参数是其它参数的分隔符。分隔符的位置放在要连接的两个字符串之间。分隔符可以是一个字符串，也可以是其它参数。如果分隔符为 NULL，则结果为 NULL。CONCAT_WS() 会忽略任何分隔符参数后的 NULL 值，但是不会忽略任何空字符串。
...
SELECT
    *
FROM base
LATERAL VIEW OUTER EXPLODE(ARRAY()) myTable1 AS myCol1
;
